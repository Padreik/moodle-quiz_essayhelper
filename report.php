...
require_once($CFG->dirroot . '/mod/quiz/report/essayhelper/gradingsettings_form.php');
...
class quiz_essayhelper_report extends quiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        if (!in_array($grade, array('all', 'needsgrading', 'manuallygraded'))) {
            $grade = null;
        }
        ...
        if (!$hasquestions) {
            ...
            $this->display_grading_interface($slot, $questionid, $grade, $page, $shownames, $showidnumbers, $order, $counts);
        }
        return true;
    }
    ...
    protected function base_url() {
        return new moodle_url('/mod/quiz/report.php',
            array('id' => $this->cm->id, 'mode' => 'essayhelper'));
    }
    ...
    protected function display_index() {
        ...
        foreach ($statecounts as $counts) {
            ...
            if ($counts->needsgrading == 0 && $counts->manuallygraded == 0) {
                continue;
            }
    ...
    protected function display_grading_interface($slot, $questionid, $grade, $page, $shownames, $showidnumbers, $order, $counts) {
        global $OUTPUT;

        if ($page >= $counts->$grade) {
            $page = 0;
        }

        list($qubaids, $count) = $this->get_usage_ids_where_question_in_state(
            $grade, $slot, $questionid, $order, $page);
        ...
        $a = new stdClass();
        $a->from = $page + 1;
        $a->to = min(($page + 1), $count);
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'quiz_grading', $a), 3);

        if ($count > 1 && $order != 'random') {
            echo $OUTPUT->paging_bar($count, $page, 1,
    ...
    protected function get_usage_ids_where_question_in_state($summarystate, $slot,
                                                             $questionid = null, $orderby = 'random', $page = 0) {
        global $CFG, $DB;
        $dm = new question_engine_data_mapper();
        ...
        return $dm->load_questions_usages_where_question_in_state($qubaids, $summarystate,
            $slot, $questionid, $orderby, $params, $limitfrom, 1);
    }
}